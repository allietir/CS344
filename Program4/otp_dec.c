...
* Date:			12/5/2019
* Description:	Client that can only connect to  the otp_dec_d server.
				It sends an encrypted file to the server be decrypted 
				using a key. It then receives the decrypted file back from
...
int main(int argc, char* argv[])
{
	...
	{
		fprintf(stderr, "CLIENT: ERROR otp_enc_d refused connection by otp_dec at port %d\n", portNumber);
		exit(2);
	}	
	...
	{
		error("CLIENT: ERROR otp_dec send messagesize, no connection to server\n");
		exit(2);
	}
	...
	while(sentSize <= messageSize)
	{
		...
		{
			error("CLIENT: ERROR otp_dec send message, no connection to server\n");
			exit(2);
		}
	...
	{
		error("CLIENT: ERROR otp_dec send keysize, no connection to server\n");
		exit(2);
	}
	...
	while(sentSize <= keySize)
	{
		...
		{
			error("CLIENT: ERROR otp_dec send key, no connection to server\n");
			exit(2);
		}
		sentSize = sentSize + 1023;
	}
