...
* Date:			12/5/2019
* Description:	Server file that accepts a connection from the otp_enc client,
				receives a file, encrypts it, and sends it back to the client.
************************************************/
...
int main(int argc, char* argv[])
{
	...
	//the largest file in the script has 70,000 characters
	char buffer[80000];
	char key[80000];
	char encrypted[80000];
		...
		switch(spawnpid)
		{
			...
			case 0: ;
				...
				if(strcmp(buffer, "otp_enc") != 0)
				{
					...
					error("SERVER: ERROR otp_enc_d refused client connection\n");
					exit(2);
				}
				else
				{
				...
				messageSize = recv(newConnection, buffer, sizeof(buffer) - 1, 0);
				if ( messageSize == -1)
				{
				...
				keySize = recv(newConnection, key, sizeof(key) - 1, 0);
				if (keySize == -1)
				{
				...
				send(newConnection, encrypted, sizeof(encrypted), 0);
