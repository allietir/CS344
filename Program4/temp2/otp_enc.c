...
int main(int argc, char* argv[])
{
	...
	fprintf(stderr, "key size stored is %d\n", keySize);
	...
	while(sentSize <= messageSize)
	{
		...
		strncpy(sent, &buffer[sentSize], 1023);
		fprintf(stderr, "sending key: %s\n", key);
		//null terminate the message
		sent[1024] = '\0';

		//send it
		if(send(socketFD, &sent, 1024, 0) == -1)
		{
			error("CLIENT: ERROR no connection to server\n");
			exit(2);
		}
	...
	while(sentSize <= keySize)
	{
		...
		fprintf(stderr, "sending key: %s\n", key);
		//sending a part of the message at a time
		strncpy(sent, &key[sentSize], 1023);
		//fprintf(stderr, "sending key: %s\n", sent);
	...
	{	
		...
		//r = recv(socketFD, sent, sizeof(sent), 0);
